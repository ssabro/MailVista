...
import { Lock, Mail, Unlock, AlertCircle } from 'lucide-react'
...
export function LockScreen({ onUnlock, pinEnabled = false }: LockScreenProps) {
  ...
  // 아무 키나 누르면 잠금 해제 (PIN 미사용 시)
  useEffect(() => {
    if (pinEnabled) return // PIN 모드에서는 키보드 잠금 해제 비활성화
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center space-y-8">
        ...
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            {pinEnabled ? t('pin.enterPin') : t('lockScreen.locked')}
          </p>
        </div>
      ...
      <div className="absolute bottom-8 text-center">
        <p className="text-xs text-muted-foreground">
          {pinEnabled ? t('pin.securityNote') : t('lockScreen.autoLockMessage')}
        </p>
      </div>
